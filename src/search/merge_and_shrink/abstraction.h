...
class Abstraction {
    ...
    bool apply_label_inheritance(const LabelReducer* reducer, bool new_approach = false);
    void comp_tc(AbstractStateRef v, int* current_index, vector<int>& index, vector<AbstractStateRef>& nstack, vector<int>& cstack, vector<int>& C, vector<int>& root, vector<vector<AbstractStateRef> >& successors, int* component_count, vector<set<int> >& csucc, EquivalenceRelation& sccs);
...
public:
    ...
    void compute_distances(bool prune_unreachable, bool prune_irrelevant, bool continue_if_unreachable = false, bool use_uniform_costs = false, bool force_recomputations = false);
    ...
    int set_goal_leading_actions(vector<bool>& labels, bool backward_pruning=false) const;
    int set_shortest_path_relevant_actions(vector<bool>& labels, bool backward_pruning = false);
    int set_path_relevant_actions(vector<bool>& labels, bool backward_pruning = false);
    ...
    const std::vector<const Operator *> &get_relevant_operators() const {
...
protected:
    ...
    virtual void apply_abstraction_to_lookup_table(const vector<AbstractStateRef> &abstraction_mapping);

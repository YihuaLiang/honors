...
protected:
    ...
    void compute_conflict_set(Fluent &subgoal); //calculate the conflict
    ...
    virtual RefinementResult refine( // the main refine function
            const std::vector<State> &root_component,
            const std::unordered_set<StateID> &recognized_neighbors);
public:
    UCRefinementOnRN(const Options &opts); // construct function

...
UCHeuristic::UCHeuristic(const Options &opts) //initialized  from the options
    : HCHeuristic(opts), // initialized with h^C heuristic
      c_eval_hc(opts.get<bool>("eval_hc")),
      c_reeval_hc(opts.get<bool>("reeval_hc")),
      m_clause_store(NULL), // no clause stored
      m_clause_extraction(NULL) // no clause extracted
{
    cost_type = OperatorCost::ONE; // the cost is 1
...
UCHeuristic::UCHeuristic(const UCHeuristic &uc)
  : HCHeuristic(uc),// Initialized from another heuristic

...
typedef std::pair<int, int> Fact;// what is meaning of two parameter
typedef std::set<Fact> Fluent; //

class Operator;
class State;
// quite different from the algorithms
...
// no direct pointers to te strips actions 
struct Conjunction {
  /*
...
class HCHeuristic : public Heuristic
{
 protected:
  ...
  std::vector<Fluent> _fluents;// set of sets of facts
  ...
  void relaxed_exploration(); //explore method
 ...
 public:
  ...
  bool extract_mutex(const Fluent &goal, Conflict &mutex);// is called
  bool extract_mutex(const Fluent &goal, Fluent &mutex);// is called in PCR
  //void update_mutex();

...
{
  Fluent fluent;
  fluent.insert(fact);//construct a fluent and merge this fluent to 
  merge(fluent);
}
...
// It seem the search algorithm is eager search
...
HCHeuristic::HCHeuristic(const Options &opts)
  : Heuristic(opts),
    ...
    c_dual_task(opts.get<bool>("dual")), // what does dual tasks actually mean
    ...
    c_update_mutex(opts.get<bool>("update_mutex"))
{
  ...
  hard_size_limit = true; // what is this 
 ...
 *  hc is a kind of hmax, it will take the max value in the recursion instead of the sum.
 */
void HCHeuristic::compile_strips()
{
  cout << "Constructing Pi^C for h^max computation ..." << endl;
  // change all things in the frame work to the final input
...
// check whether a set of facts is in a state
...
// queue operation - the conjunction 
 ...
 *
 */
// it should be DFS but it looks like BFS???
// Using queue to replace the recursion is quite unfamiliar to me ....
// this is the searching process

/*
* Consider how to change the function and other relevant function below 
*/
//计算启发值
//require set up at first
void HCHeuristic::relaxed_exploration()
{
  ...
  while (!queue.empty()) {
    ...
    for (uint i = 0; i < triggered_counters.size(); i++) {
      ...
      if (--counter->unsatisfied_preconditions == 0) { // all precondition are satisfied -- include the cost
        // change this condition here to check cost??
        counter->cost = conj->cost;// operating the actions 
        enqueue_if_necessary(counter->effect,
                             counter, counter->base_cost + counter->cost); // accummulating the cost
      }
    }
  }
}
...
// this function has been reloaded for different type of state 
int HCHeuristic::simple_traversal_setup(const std::vector<std::pair<int, int> >
                                        &state,
                                        std::vector<unsigned> &exploration)//a queue
{
...
{ //Fluent is a set of facts -- from begin to end -- check the variable
  for (Fluent::const_iterator i = f.begin(); i != f.end(); i++) {
    Fluent::const_iterator j = i;
    while (++j != f.end()) {
      if (are_mutex(*i, *j)) {

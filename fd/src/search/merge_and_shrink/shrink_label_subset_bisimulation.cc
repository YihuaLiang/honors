...
ShrinkLabelSubsetBisimulation::ShrinkLabelSubsetBisimulation(const Options &opts)
...
//	cout << "Bisimulation type: ";
    ...
    else if (goal_leading == SHORTEST_PATH_RELEVANT_FROM_THRESHOLD_BACKWARD_PRUNED)
...
void ShrinkLabelSubsetBisimulation::dump_caught_labels_statistic() const {
	...
	cout << "Using goal leading actions "<< to_catch << " out of total number of actions " << g_operators.size() << endl;
...
void ShrinkLabelSubsetBisimulation::shrink_before_merge(
    ...
    if (!goal_leading_threshold_reached && abs1.size() > max_states_before_catching) {
...
const std::vector<bool> &ShrinkLabelSubsetBisimulation::get_caught_labels_set() const
{
	if (goal_leading == APPROXIMATED_BY_H_MAX ||
			goal_leading == APPROXIMATED_BY_H_MAX_BACKWARD_PRUNED ||
			goal_leading == APPROXIMATED_BY_H_FF)
...
void ShrinkLabelSubsetBisimulation::handle_option_defaults(Options &opts) {
...
static ShrinkStrategy *_parse(OptionParser &parser) {
    ...
    parser.add_option<bool>("use_all_best_supporters", "for approximation by h_max: "
    		"should we use all best supporters, or only one for each fact?", "true");

    parser.add_option<int>("max_states_before_catching",
    		"the limit at which to calculate the threshold-based label sets", std::to_string(numeric_limits<int>::max()));

    Options opts = parser.parse();
    ShrinkLabelSubsetBisimulation::handle_option_defaults(opts);

    if (!parser.dry_run())
        return new ShrinkLabelSubsetBisimulation(opts);
    else
        return 0;
}

static Plugin<ShrinkStrategy> _plugin("shrink_label_subset_bisimulation", _parse);

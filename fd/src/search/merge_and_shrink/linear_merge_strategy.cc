...
LinearMergeStrategy::LinearMergeStrategy(const std::vector <MergeCriterion *> & merge_criteria,
					 const MergeOrder & merge_order, bool is_first) :
  criteria(merge_criteria), order(merge_order) {
...
void MergeCriterionGoal::filter(std::vector <int> & vars, Abstraction * /*abstraction*/) {
...
void MergeCriterionMinSCC::forbid_scc_descendants(int scc_index,
  ...
  for(set<int>::iterator it = descendants.begin(); it != descendants.end(); ++it){
...
void MergeCriterionMinSCC::filter(std::vector <int> & vars, Abstraction * abstraction) {
  if(!abstraction) return;
  if (!allow_incremental){
    is_causal_predecessor.resize(g_variable_domain.size(), false);
    const vector<int> & varset = abstraction->get_varset();
...
void MergeCriterionEmpty::filter(std::vector <int> & vars, Abstraction * abstraction) {
...
void MergeCriterionEmptyGoal::filter(std::vector <int> & vars, Abstraction * abstraction) {
  if(abstraction){ //Check if abstraction exists, because the first variable is selected without abstraction
    const vector<double> & score = abstraction->get_count_transitions_var_empty_goal();
    // for(int i = 0; i < vars.size(); i++)
...
static Plugin<MergeCriterion> _plugin_empty("empty", _parse<MergeCriterionEmpty>);
static Plugin<MergeCriterion> _plugin_empty_goal("empty_goal", _parse<MergeCriterionEmptyGoal>);

...
public:
    ...
    void set_labels_from_abstraction(Abstraction &abs, bool backward_pruning = false, int find_goal_leading_actions = 0);
    void set_labels(std::vector<bool>& labels_);

    static void add_options_to_parser(OptionParser &parser);
    static void handle_option_defaults(Options &opts);

    virtual const std::vector<bool> &get_caught_labels_set() const {return labels;}

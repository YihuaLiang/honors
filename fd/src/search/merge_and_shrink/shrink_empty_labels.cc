...
ShrinkEmptyLabels::ShrinkEmptyLabels(const Options &opts) : ShrinkStrategy(opts) {
}
...
void ShrinkEmptyLabels::shrink(Abstraction &abs, int target, bool /*force*/) {
	...
	const vector<const Operator*> &relevant_ops = abs.get_relevant_operators();
	for (size_t op_no = 0; op_no < relevant_ops.size(); op_no++) {
		if (!empty_label_operators[op_no])
			continue;
		const vector<AbstractTransition> &transitions = abs.get_transitions_for_op(get_op_index(relevant_ops[op_no]));
    ...
    for (size_t i = 0; i < num_states; i++) {
    	if (indices[i] == -1)
    		...
    		::find_sccs(stack, in_stack, final_sccs, adjacency_matrix, indices, lowlinks, index, i, abs.get_all_goal_vars_in(), is_goal, state_to_scc);
    }
...
void ShrinkEmptyLabels::determine_empty_label_operators(Abstraction& abs, vector<bool>& empty_label_operators) {
	 ...
	 */
    for (size_t op_no = 0; op_no < relevant_ops.size(); ++op_no) {
        const Operator *op = relevant_ops[op_no];
...
ShrinkEmptyLabels *ShrinkEmptyLabels::create_default() {
	const int infinity = numeric_limits<int>::max();
    Options opts;
    opts.set("max_states", infinity);
    opts.set("max_states_before_merge", infinity);
    return new ShrinkEmptyLabels(opts);
}

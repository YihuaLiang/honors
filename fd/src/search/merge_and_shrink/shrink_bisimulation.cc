...
void ShrinkBisimulation::set_labels_from_abstraction(Abstraction &abs, bool backward_pruning, int type) {
	labels.clear();
	if (type == 0)
		...
		cerr << "unknown type " << type << " for setting labels from abstraction" << endl;
		exit(1);
	}
}
...
ShrinkStrategy *ShrinkBisimulation::create_default() {
    ...
    opts.set("greedy", NOT_GREEDY);
    opts.set("threshold", 1);
    opts.set("initialize_by_h", true);
    opts.set("group_by_h", false);
    opts.set("at_limit", RETURN);

    return new ShrinkBisimulation(opts);
}

void ShrinkBisimulation::add_options_to_parser(OptionParser &parser) {
...
void ShrinkBisimulation::handle_option_defaults(Options &opts) {

...
MergeAndShrinkHeuristic::MergeAndShrinkHeuristic(const Options &opts)
    : Heuristic(opts),
      ...
      merge_criteria(opts.get_list<MergeCriterion *>("merge_criteria")),
...
void MergeAndShrinkHeuristic::dump_options() const {
  ...
  cout << endl;
    ...
    if (use_label_inheritance) {
    	cout << "Using the " << (new_approach ? "new" : "old") << " label inheritance implementation" << endl;
    } else {
...
void MergeAndShrinkHeuristic::get_candidate_vars(vector<int> & res,
  ...
  for (int i = 0; i < g_variable_domain.size(); ++i){
    //a) Skip variables already in the abstraction
    if(vset.count(i)) continue;

    //  b) Skip irrelevant variables
    if (atomic_abstractions[i]->is_irrelevant()) continue;

    //c) Skip merging variables whose size is surpassed
    if(abstraction && abstraction->size()*atomic_abstractions[i]->size() > merge_size_limit){
...
void MergeAndShrinkHeuristic::prepare_after_merge(Abstraction * abstraction,
	...
	abstraction->normalize(use_label_reduction, shrink_strategy->get_caught_labels_set(),
	...
	if (shrink_strategy->reduce_labels_before_shrinking() || use_label_inheritance) {
	  abstraction->normalize(false/*use_label_reduction*/ /*no need to reduce labels again -- transitions with same labels are already merged!*/, shrink_strategy->get_caught_labels_set(),
				 use_label_inheritance, new_approach);
      ...
      abstraction->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable, use_uniform_distances);
      // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
      //abstraction->statistics(use_expensive_statistics);
...
void MergeAndShrinkHeuristic::prepare_before_merge(Abstraction * abstraction, Abstraction * other_abstraction) const{
  if (shrink_strategy->reduce_labels_before_shrinking() || use_label_inheritance || empty_label_shrink_strategy) {
    // Note: label inheritance for atomic abstractions works only for linear merging strategies,
    // as the label set cannot be empty for the MAIN abstraction
    other_abstraction->normalize(false, shrink_strategy->get_caught_labels_set(),
				 use_label_inheritance, new_approach);
  ...
  other_abstraction->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable, use_uniform_distances);
  // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
  //other_abstraction->statistics(use_expensive_statistics);
#ifdef DEBUG_ABSTRACTIONS
  ...
  */
  ...
  abstraction->normalize(use_label_reduction, shrink_strategy->get_caught_labels_set(), false);
  // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
  //abstraction->statistics(use_expensive_statistics);

  // Don't label-reduce the atomic abstraction -- see issue68.
  other_abstraction->normalize(false, shrink_strategy->get_caught_labels_set(), false);
  // Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
  //other_abstraction->statistics(use_expensive_statistics);
}

//As build abstraction, but calling to the dfs procedure instead
void MergeAndShrinkHeuristic::build_multiple_abstractions(vector<Abstraction *> & res_abstractions) {
  ...
  MultipleLinearMergeStrategy order (merge_criteria, merge_order, true /*is_first*/);
  ...
  for(int i = 0; i < selected_variables.size() && i <= max_branching_merge; i++){
    ...
    if(build_abstraction_dfs(abstraction, order, explored_varsets, res_abstractions)){
      return;
    }
  }
}
...
bool MergeAndShrinkHeuristic::build_abstraction_dfs(
  ...
  if (selected_variables.empty()){
    ...
    for(int i = 0; i < selected_variables.size(); i++){
      int var_no = selected_variables[i];
      ...
      if (prune_unreachable || prune_irrelevant || shrink_strategy->require_distances()) {
	new_abstraction->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable, use_uniform_distances);
...
void MergeAndShrinkHeuristic::build_atomic_abstractions() {
  Abstraction::build_atomic_abstractions(is_unit_cost_problem(),
  ...
  for (size_t i = 0; i < atomic_abstractions.size(); ++i) {
    ...
    if (!atomic_abstractions[i]->is_irrelevant()) {
      atomic_abstractions[i]->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable, use_uniform_distances);
      if (!atomic_abstractions[i]->is_solvable() && !continue_if_unsolvable) {
	cout << "Number of merging steps: 0" << endl;
	cout << "Last abstraction step: -1/" << g_variable_domain.size() << " vars" << endl;
	return;
      }
...
Abstraction *MergeAndShrinkHeuristic::build_abstraction(bool is_first) {
    ...
    Abstraction::build_atomic_abstractions(
        is_unit_cost_problem(), get_cost_type(), atom_abstractions);
    ...
    for (size_t i = 0; i < atom_abstractions.size(); ++i) {
    	...
    	atom_abstractions[i]->compute_distances(prune_unreachable, prune_irrelevant, continue_if_unsolvable, use_uniform_distances);
    ...
    for(int i = 0; i < g_fact_names[var_no].size(); ++i)
    ...
    while (!order->done()) {
      cout << "DFS1: " << *abstraction << endl;
        // TODO: When using nonlinear merge strategies, make sure not
        // to normalize multiple parts of a composite. See issue68.
      if (shrink_strategy->reduce_labels_before_shrinking() || order->reduce_labels_before_merge()
	  || use_label_inheritance || empty_label_shrink_strategy) {
	...
	if (!abstraction->is_solvable() && !continue_if_unsolvable) {
	  ...
	  cout << "Last abstraction step: " << (merge_count + 1) << "/" << g_variable_domain.size() << " vars" << endl;
	  delete order;
	  return abstraction;
	}
    	// Peter: number of states and/or transitions cannot have increased, so no need to update statistics here
	...
	cout << endl;
	  ...
	  // Note: label inheritance for atomic abstractions works only for linear merging strategies,
        ...
        */
        ...
        int peak_memory = max(abstraction->get_peak_memory_estimate(), other_abstraction->get_peak_memory_estimate());
        int max_abstract_states = max(abstraction->get_max_num_states(), other_abstraction->get_max_num_states());
        int max_abstract_transitions = max(abstraction->get_max_num_transitions(), other_abstraction->get_max_num_transitions());
        int overall_peak_memory = max(abstraction->get_overall_peak_memory_estimate(), other_abstraction->get_overall_peak_memory_estimate());
        int overall_max_abstract_states = max(abstraction->get_overall_max_num_states(), other_abstraction->get_overall_max_num_states());
        int overall_max_abstract_transitions = max(abstraction->get_overall_max_num_transitions(), other_abstraction->get_overall_max_num_transitions());
        cout << "Estimated peak memory for this abstraction: " << peak_memory << " bytes" << endl;
        cout << "Maximal number of states in this abstraction: " << max_abstract_states << endl;
        cout << "Maximal number of transitions in this abstraction: " << max_abstract_transitions << endl;
...
void MergeAndShrinkHeuristic::initialize() {
    ...
    if(merge_strategy == MERGE_LINEAR_MULTIPLE_DFS){
      ...
      for (int i = 0; i < abstraction_count; i++) {
        ...
        max_abstract_states = max(max_abstract_states, abstraction->get_overall_max_num_states());
        max_abstract_transitions = max(max_abstract_transitions, abstraction->get_overall_max_num_transitions());
    ...
    cout << "Maximal number of states in abstractions: " << max_abstract_states << endl;
    cout << "Maximal number of transitions in abstractions: " << max_abstract_transitions << endl;
...
static ShrinkStrategy *_default_shrink(OptionParser &/*parser*/)
{
    return ShrinkFH::create_default(50000);
}

static Plugin<ShrinkStrategy> _pluginsas("default", _default_shrink);
...
static Heuristic *_parse(OptionParser &parser) {
    ...
    parser.add_enum_option("merge_order", merge_orders, "merge variable order for tie breaking",
			   "RANDOM");

    // TODO: Default shrink strategy should only be created
    // when it's actually used.
    parser.add_option<ShrinkStrategy *>("shrink_strategy", "shrink strategy", "default");

    parser.add_list_option<MergeCriterion *> ("merge_criteria", "merge strategy criteria", "[]");
    ...
    parser.add_option<bool>("prune_unreachable", "prune unreachable states (WARNING: setting this to false it not fully supported)", "true");
    parser.add_option<bool>("prune_irrelevant", "prune irrelevant states (WARNING: setting this to false it not fully supported)", "true");
    ...
    parser.add_option<int>("merge_size_limit",
			   "Maximum size of abstractions after a merge in MERGE_LINEAR_MULTIPLE_DFS", "10000");

    parser.add_option<int>("max_branching_merge",
			   "Maximum branching factor in MERGE_LINEAR_MULTIPLE_DFS", std::to_string(numeric_limits<int>::max()));

...
namespace lm_cut_heuristic {
  ...
  LandmarkCutLandmarks::LandmarkCutLandmarks(OperatorCost cost_type) : cost_type(cost_type) {
    ::verify_no_axioms();
    ...
    for (uint op = 0; op < g_operators.size(); op++) {
      build_relaxed_operator(g_operators[op], op);
    }
    ...
    for (const auto & goal : g_goal) {
      goal_op_pre.push_back(get_proposition(goal.first, goal.second));
    }
  ...
  void LandmarkCutLandmarks::build_relaxed_operator(const Operator &op, int op_id) {
    vector<RelaxedProposition *> precondition;
    vector<RelaxedProposition *> effects;
    for (const auto & prev : op.get_prevail()) {
      precondition.push_back(get_proposition(prev.var, prev.prev));
    }
    for (const auto & eff : op.get_pre_post()) {
      if (eff.pre != -1) {
        precondition.push_back(get_proposition(eff.var, eff.pre));
      }
      effects.push_back(get_proposition(eff.var, eff.post));
    }
    add_relaxed_operator(
                         move(precondition), move(effects), op_id, get_adjusted_action_cost(op, cost_type));
  }
  ...
  RelaxedProposition *LandmarkCutLandmarks::get_proposition(int var_id, int val) {
  ...
  void LandmarkCutLandmarks::setup_exploration_queue_state(const State &state) {
    for (uint var = 0; var < g_variable_domain.size(); var++) {
      enqueue_if_necessary(get_proposition(var, state[var]), 0);
    }
  ...
  void LandmarkCutLandmarks::second_exploration(
    ...
    for (unsigned var = 0; var < g_variable_domain.size(); var++) {
      RelaxedProposition *init_prop = get_proposition(var, state[var]);

...
void LandmarkGraph::dump_graphviz(set<int> edge_types, ostream &out) const {
    out << "digraph G {" << endl;

    if (edge_types.empty()) {
        edge_types.insert(necessary);
        edge_types.insert(greedy_necessary);
        edge_types.insert(natural);
        edge_types.insert(reasonable);
        edge_types.insert(obedient_reasonable);
    }

    set<LandmarkNode *, LandmarkNodeComparer> nodes2(nodes.begin(), nodes.end());
    map<const LandmarkNode *, size_t> node_num;
    ...
    for (set<LandmarkNode *>::const_iterator it = nodes2.begin(); it
         != nodes2.end(); it++) {
        LandmarkNode *node_p = *it;
        for (set<int>::iterator et = edge_types.begin(); et != edge_types.end();
                et++) {
            set<LandmarkNode*, LandmarkNodeComparer> edgs;
            for (hash_map<LandmarkNode *, edge_type, hash_pointer>::const_iterator
                 child_it = node_p->children.begin(); child_it
                 != node_p->children.end(); child_it++) {
                const edge_type &edge = child_it->second;
                if (edge == *et) {
                    edgs.insert(child_it->first);
                }
            }
            for (set<LandmarkNode*>::iterator child_it = edgs.begin();
                    child_it != edgs.end(); child_it++) {
                const LandmarkNode *child_p = *child_it;
                out << "lm_node_" << node_num[node_p] << " -> "
                    << "lm_node_" << node_num[child_p] << " [label=\"";
                switch (*et) {
                case necessary:
                    out << "nec";
                    break;

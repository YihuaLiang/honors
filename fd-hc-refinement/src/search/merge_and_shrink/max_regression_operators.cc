...
MaxRegressionOperators::MaxRegressionOperators(const Options &opts)
    : HSPMaxHeuristic(opts), alpha(opts.get<double>("alpha")), all_best_supporters(opts.get<bool>("use_all_best_supporters")) {
...
//    cout << "==================== Operators==============" << endl;
	...
	cout << "Heuristic value: " << get_value() << ", alpha: " << alpha << ", calculated bound: " << bound << endl;
...
void MaxRegressionOperators::mark_relaxed_operators(vector<bool>& labels, double bound) {
	int num_props = 0;
    ...
    for (int i = 0; i < unary_operators.size(); i++) {
		...
		if (op->effect->cost == -1 ||
				((bound > 0) and (op->effect->cost >= bound)))
			continue;
        ...
        if (all_best_supporters)
        	...
        	const vector<PrePost> &prepost = g_operators[op->operator_no].get_pre_post();
...
//		dump_unary_operator(op);
	...
	mark_relevant_operators(num_props, outgoing_operators, reached_propositions, labels, num_marked_operators);

}



void MaxRegressionOperators::mark_minimal_set_of_relaxed_operators(vector<bool>& labels) {
	...
	//		dump_unary_operator(op);
        ...
        int num_marked_operators = 0;
    	mark_relevant_operators(num_props, achieving_operators, achieved_propositions, labels, num_marked_operators);
    }

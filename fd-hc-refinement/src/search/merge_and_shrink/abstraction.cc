...
void Abstraction::compute_distances(bool prune_unreachable, bool prune_irrelevant, bool continue_if_unsolvable, bool use_uniform_costs, bool force_recomputation) {
    cout << tag() << flush;
...
void Abstraction::compute_goal_distances_unit_cost() {
    ...
    deque<AbstractStateRef> queue;
    for (AbstractStateRef state = 0; state < num_states; state++) {
        if (goal_states[state] && init_distances[state] < infinity) { // second condition: state not proven irrelevant
...
void find_sccs(
		...
		vector<slist<AbstractStateRef> >& final_sccs,
	...
	for (size_t i = 0; i < successors.size(); i++) {
		...
		if (indices[succ] == -1) {
			find_sccs(stack, in_stack, final_sccs, adjacency_matrix, indices, lowlinks, index, succ, all_goal_vars_in, is_goal, state_to_scc);
	...
	if (lowlinks[state] == indices[state]) {
		slist<AbstractStateRef> new_scc;
...
bool Abstraction::is_scc() const {
	...
	find_sccs(stack, in_stack, final_sccs, adjacency_matrix, indices, lowlinks, index, init_state, all_goal_vars_in, is_goal, state_to_scc, true);
	if (final_sccs[0].size() == num_states) // all states in a single SCC
		return true;
	return false;
}
...
void Abstraction::comp_tc(
	...
	for (int i = 0; i < v_succs.size(); i++) {
		AbstractStateRef w = v_succs[i];
		bool w_unvisited = false;
		if (C[w] == -2) {
			w_unvisited = true;
			(*current_index)++;
			comp_tc(w, current_index, index, nstack, cstack, C, root, successors, component_count, csucc, sccs);
		}
		if (C[w] == -1) {
...
bool Abstraction::apply_label_inheritance(const LabelReducer* reducer, bool new_approach) {
...
//#ifdef NEW_APPROACH
    ...
    vector<StateBucket> all_successors(num_states);
	...
	for (AbstractStateRef v = 0; v < num_states; v++) {
		if (C[v] == -2) {
			comp_tc(v, &current_index, index, nstack, cstack, C, root, successors, &component_cout, csucc, sccs);
			current_index++;
		}
	}
    ...
    for (int s = 0; s < num_states; s++) {
    	...
    	while (csucc_it != csucc_set.end()) {
    		...
    		/*const StateBucket &all_succs = all_successors[succ];
    ...
    /* free some memory */
    vector<StateBucket> ().swap(all_successors);
	vector<int> ().swap(C);
	vector<set<int> > ().swap(csucc);
	EquivalenceRelation ().swap(sccs);
...
//#else
	...
	empty_label_transition.resize(transitions_by_op.size(), true);
    typedef vector<pair<AbstractStateRef, int> > StateBucket;
    vector<StateBucket> successors(num_states);
    ...
    vector<bool> ().swap(empty_label_transition);
    vector<StateBucket> ().swap(successors);
	}
//#endif
...
void CompositeAbstraction::apply_abstraction_to_lookup_table(const vector<AbstractStateRef> &abstraction_mapping) {
    cout << tag() << "applying abstraction to lookup table" << endl;
...
void Abstraction::normalize(bool reduce_labels, const std::vector<bool> &caught_labels,
    ...
    */
    ...
    if (reduce_labels) {
        if (are_labels_reduced) {
            ...
            reducer = new LabelReducer(relevant_operators, varset, cost_type, caught_labels, vars_label_operators);
        }
    } else {
...
CompositeAbstraction::CompositeAbstraction(
    ...
    init_state = PRUNED_STATE; // default value -- in case it is pruned in one of the abstractions
...
void Abstraction::dump(ostream &ou) const {
    ...
    for (int src = 0; src < num_states; src++) {
        for (int target = 0; target < num_states; target++) {
            ...
            for (int op_no=0; op_no < ops.size(); op_no++) {
            	ou << "o_" << ops[op_no] <<
...
std::ostream & operator << (std::ostream & ss, const Abstraction & abs){
  ss << "mas[";
  for (int i = 0; i < abs.varset.size(); i++){
    ss << abs.varset[i] << " ";
  }
  return ss << "]";
}
...
int Abstraction::set_goal_leading_actions(vector<bool>& labels, bool backward_pruning) const {
	Timer timer;
    ...
    //for (int op_no = 0; op_no < g_operators.size(); op_no++) {
        ...
        if (last_computation_used_uniform_costs)
        	...
        	op_cost = get_adjusted_action_cost(g_operators[op_no], cost_type); // Peter: TODO Must do something about this!!!
...
//        	") to " << target << " (h*="<< target_g <<") with cost " << op_cost << endl;
    ...
    cout << "Dumped goal leading actions "<< num_goal_leading << " out of total number of actions " << g_operators.size() << endl;
    cout << "Finding label set took: " << timer << endl;

    return num_goal_leading;
}

int Abstraction::set_shortest_path_relevant_actions(vector<bool>& labels, bool backward_pruning) {
    Timer timer;
    ...
    cout << "Dumped shortest-path relevant actions "<< num_actions << " out of total number of actions " << g_operators.size() << endl;
    cout << "Finding label set took: " << timer << endl;

    return num_actions;
}

int Abstraction::set_path_relevant_actions(vector<bool>& labels, bool /*backward_pruning*/) {
    Timer timer;
...
void Abstraction::write_mip_file(const bool backward_pruning, const bool use_uniform_costs) {
	cout << "writing MIP file for finding outgoing actions in " << (use_uniform_costs ? "unit-cost" : "original") << " setting..." << endl;
	...
	ofstream out(use_uniform_costs ? "necessary_actions_uniform.cplex" : "necessary_actions.cplex");
	out << "Minimize" << endl;
    if (g_operators.size() > 0) {
        out << "   ";
        for (int i = 0; i < g_operators.size() - 1; i++) {
            out << "A" << i << " + ";
        }
	...
	for (int op_no = 0; op_no < g_operators.size(); op_no++) {
		const vector<AbstractTransition> &transitions = transitions_by_op[op_no];
        int op_cost = (use_uniform_costs ? 1 : get_adjusted_action_cost(g_operators[op_no], cost_type));
		for (int i = 0; i < transitions.size(); i++) {
			const AbstractStateRef &src = transitions[i].src;
            int src_g = get_goal_distance(src);
            if ((src_g == infinity) ||
...
//        	") to " << target << " (h*="<< target_g <<") with cost " << op_cost << endl;
	...
	for (int i = 0; i < num_states; i++) {
		...
		if (!outgoing_transitions[i].empty()) {
			out << "   S" << i << ": ";
			for (int j = 0; j < outgoing_transitions[i].size() - 1; j++) {
				out << "A" << outgoing_transitions[i][j] << " + ";
			}
			out << "A" << outgoing_transitions[i][outgoing_transitions[i].size() - 1] << " >= 1" << endl;
		}
	}
	/*out << "Bounds" << endl;
	for (int i = 0; i < g_operators.size(); i++) {
...
void Abstraction::write_pmaxsat_file(const bool backward_pruning) {
	cout << "writing partial MaxSAT file for finding actions preserving optimal solutions" << endl;
	...
	*/
	...
	//if (backward_pruning) {
		...
		vector<slist<AbstractStateRef> > remainingStates;
		...
		for (int i = 0; i < num_states; i++) {
			...
			slist<AbstractStateRef> nextState;
			nextState.push_front(i);
			remainingStates.push_back(nextState);
		}
		apply_abstraction(remainingStates);
...
//        	") to " << target << " (h*="<< target_g <<") with cost " << op_cost << endl;
	...
	for (int s = 0; s < num_states; s++) {
		out << top << " " << (goal_states[s] ? "" : "-") << (offset + s) << " 0" << endl;
	}
	...
	for (int s = 0; s < num_states; s++) {
		for (int k = 1; k <= maxDist; k++) {
			...
			for (int t = 0; t < transitions.size(); t++) {
				out  << " " << (offset_t_variables + (k - 1) * num_transitions + transitions[t]);
			}
			out << " 0" << endl;
		}
	}
	...
	trans_index = 0;
	for (int o = 0; o < g_operators.size(); o++) {

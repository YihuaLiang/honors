...
HeuristicRefiner::RefinementResult UCRefinementOnRN::refine(const std::vector<State> &states,
                            const std::unordered_set<StateID> &rn)// what is the second parameter
{
    std::unordered_set<StateID> mapped_rn;
    (*m_rn_map)(uc, rn, mapped_rn);

    if (prepare_refinement(states, mapped_rn)) {
        return UNCHANGED; // don't need to change
    }

    Fluent goal;// setup the goal
    for (uint i = 0; i < g_goal.size(); i++) {
        goal.insert(g_goal[i]);
    }
    //calculate the conflict
    compute_conflict_set(goal); // this is could be extract function
    //update c with _conflicts
                            ...
                            const std::unordered_set<StateID> &rn)
{
    ...
    uc->evaluate(states[0]);// calculate the heuristic on the state
    uc->set_early_termination(_early);
    if (uc->is_dead_end()) {// if dead end then no other change should be done
        return true;
    }
...
void UCRefinementOnRN::compute_conflict_set(Fluent &subgoal)
{
    ...
    // catch the conflict size and enlarge the conflict size
    ...
    for (unsigned act = 0; act < can_add.size(); act++) { // go throuth the whole vector
        // NOTE that the conflict reference might become invalid due to vector
        // reallocation when going into recursion!
        if (can_add[act] != 1) {
            continue;
        }
        const StripsAction &action = uc->get_action(act);// get action by the act number

        Fluent pre;
        pre.insert(action.precondition.begin(), action.precondition.end());// insert the whole precondition to pre
        fluent_op::set_minus(_conflicts[conflict_id].get_fluent(), action.add_effect,
                             pre);//get the minus set

        //if (!uc->contains_mutex(pre)) {
            compute_conflict_set(pre);// run the recursion process.
            //} 
    }
    can_add.clear();// clear up the vector
}
